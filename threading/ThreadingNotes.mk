...
From now on, we'll just use nested classes for things like this, to make the code a bit more concise:
...
- The order will change every time we run it
...
- `Thread.sleep(long time)` is a method that throws `InterruptedException` so it has to be caught:
